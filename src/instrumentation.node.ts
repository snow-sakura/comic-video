/**
 * Node.js 运行时专用 instrumentation（Next.js 16 推荐拆分模式）
 * 仅在 NEXT_RUNTIME=nodejs 时由 instrumentation.ts 动态导入。
 * 放置于独立文件，避免 Edge Runtime 编译时静态分析到 Node.js API（process.on 等）。
 */
